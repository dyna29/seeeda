<?php 
/*
*Template Name: Edit Spare Part Template
*/ 
get_header('user');
while ( have_posts() ) : the_post(); 
	...
	if(isset($_REQUEST["id"])){
		$spare_part_id_en = pll_get_post($_REQUEST["id"],'en' );
	    $spare_part_id_ar = pll_get_post($_REQUEST["id"],'ar' );
	 
	} 
	 echo $spare_part_id_ar ;
	...
	if(isset($_REQUEST['save_spare_part'])){  
		...
		$post_update = array( 
			...
			'post_type' =>'spare_part','post_status'=>'publish','lang'=>'en',
		);
		$spare_part_id_en =  wp_update_post( $post_update );
		
		pll_set_post_language($spare_part_id_en, 'en');
		$post_update = array( 
			'ID'         => $spare_part_id_ar,
			'post_title' => $spare_part_name_ar,
			'post_type' =>'spare_part','post_status'=>'publish','lang'=>'ar',
		);
		$spare_part_id_ar =  wp_update_post( $post_update );
	...
	$model_year_ar = (int) $arabic_term_id;
	 ...
	 $file = $_FILES['featured_image'];	 
		...
		$success_msg = pll__('Spare Part Updated Successfully!');
	} 
				...
				<div class="row">
					...
					<div class="card">
						<div class="card-header card-header-primary">
							<h4 class="card-title"><?php echo pll__('Edit Spare Part');?></h4>
							<p class="card-category"></p>
						</div>
						<?php 
						$_associated_junkyard = get_post_meta( $spare_part_id_en,'_associated_junkyard',true);  
						$featuredimage = wp_get_attachment_image_src( get_post_thumbnail_id( $spare_part_id_en ), 'full' ); 	
						?>
						<div class="card-body">
...
?>
								...
								<div class="row">
									<div class="col-md-6">
										<div class="form-group">
											...
											<input type="text" class="form-control  "  name="spare_part_name" id='spare_part_name' value="<?php echo get_the_title($spare_part_id_en);?>" >
										</div>
									</div> 
									<div class="col-md-6">
										<div class="form-group">
											<label class="bmd-label-floating"><?php echo pll__('Name');?>(AR)</label>
											<input type="text" class="form-control  "  name="spare_part_name_ar" id='spare_part_name_ar' value="<?php echo get_the_title($spare_part_id_ar);?>" >
										</div>
									</div> 
 ...
 $terms = wp_get_post_terms($spare_part_id_en, 'brand', array(
...
$terms = wp_get_post_terms($spare_part_id_en, 'model-year');
...
?>
		...
		<input type="text" class="form-control  "  name="cost"id='cost'   value="<?php echo get_post_meta($spare_part_id_en,'cost',true);?>" >
		</div>
		</div>
			<div class="col-md-12">
		<div class="form-group">
...
$terms = wp_get_post_terms($spare_part_id_en, 'spare-part-type');
...
?>
	<ul class="row">
		...
		<?php
		$terms = wp_get_post_terms($spare_part_id_en, 'wheel-size');
...
?>
	<ul class="row">
		...
		<?php
				$terms = wp_get_post_terms($spare_part_id_en, 'wheel-lug-nut');
...
<?php 
 ...
 echo get_post_meta($spare_part_id_en,'wheel_type',true);
?>	 
	<ul class="row">  
		<li class="col-md-4"><input type="radio" name="wheel_type" value="Original" <?php if(get_post_meta($spare_part_id_en,'wheel_type',true)=="Original") {?>checked <?php } ?>><?php echo  pll__('Original (OEM)');?></li>
		<li class="col-md-4"><input type="radio" name="wheel_type" value="After-Market" <?php if(get_post_meta($spare_part_id_en,'wheel_type',true)=="After-Market") {?>checked <?php } ?>><?php echo  pll__('After-Market (Non-Original)');?></li>
