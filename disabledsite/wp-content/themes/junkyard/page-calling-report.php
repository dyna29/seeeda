...
while ( have_posts() ) : the_post(); 
	...
	if(isset($_REQUEST['id'])){
		...
		$success_msg = pll__('Model Deleted Successfully!');
			}
		}
	}
	?> 
		...
		<nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top ">
			<div class="container-fluid">
				<div class="navbar-wrapper">
					<a class="navbar-model" href="javascript:;"><?php echo pll__('Model');?></a>
				</div>
					...
					<div class="col-md-12">
						...
						<div class="card">
							<div class="card-header card-header-primary">
								<h4 class="card-title"><?php echo pll__('Calling Reports');?></h4>
								<p class="card-category"></p>
							</div>
							<div class="card-body">
								<div class='filter-block'></div>
								<table class="display responsive nowrap canvas_div_pdf " id='calling-report'  >
								<thead class=" text-primary">
									<tr>
										...
										?>										
										<th style='text-align:center;'>
										<?php echo pll__('Junkyard');?>
										</th> 
										<?php } ?>
										<th style='text-align:center;'>
										<?php echo pll__('Phone Calls');?>
										</th>   
										<th style='text-align:center;'>
										<?php echo pll__('Whatsapp Calls');?>
										</th>   
									</tr>
								</thead>
								<?php 
...
$query = "
    SELECT 
        junkyard,
        DATE_FORMAT(calling_date, '%d-%m-%Y') AS formatted_date,
        SUM(CASE WHEN calling_type = 'WhatsApp' THEN 1 ELSE 0 END) AS whatsapp_calls,
        SUM(CASE WHEN calling_type = 'Phone' THEN 1 ELSE 0 END) AS phone_calls
    FROM wp_callings
    GROUP BY junkyard, calling_date
    ORDER BY calling_date DESC
";
 }else{
    ...
    SELECT 
        ...
        SUM(CASE WHEN calling_type = 'WhatsApp' THEN 1 ELSE 0 END) AS whatsapp_calls,
        SUM(CASE WHEN calling_type = 'Phone' THEN 1 ELSE 0 END) AS phone_calls
    FROM wp_callings where junkyard = ".$junkyard_id."
    GROUP BY junkyard, calling_date
    ORDER BY calling_date DESC
";
 }
// Execute the query
$results = $wpdb->get_results($query, ARRAY_A); // Fetch as associative array

// Loop through the results
if (!empty($results)) {
    foreach ($results as $row) {
		$junkyard = get_the_title($row['junkyard']);
        echo "<tr><td style='text-align:center;'>{$row['formatted_date']}</td>";
				 
										if( (in_array("junkyards_administrator", $user_role))){
									 
                 echo "<td style='text-align:center;'>".$junkyard."</td>";
										}
                echo "<td style='text-align:center;'>{$row['whatsapp_calls']}</td>
                <td style='text-align:center;'>{$row['phone_calls']}</td>
              </tr>";
    }
    echo "</table>";
...
$(function () 
	...
	var table =$('#calling-report').DataTable( {
 "order": [[0, 'desc']],   
		dom: 'Bfrtip',
		buttons: [ {
		extend: 'excelHtml5',
		autoFilter: true,
		sheetName: 'Calling Report'
		}   , 
