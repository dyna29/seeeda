...
/*
*Template Name: Add Wheel Lug Nut Template
*/ 
get_header('user'); 
			$user_role = array();
...
while ( have_posts() ) : the_post(); 
	...
	if(isset($_REQUEST['save_wheel_lug_nut'])){ 
		...
		$wheel_lug_nut_en = sanitize_text_field($_REQUEST['wheel_lug_nut']);
		$wheel_lug_nut_ar = sanitize_text_field($_REQUEST['wheel_lug_nut_ar']);
 
			// Add the term to the taxonomy
			$new_term = wp_insert_term(
				$wheel_lug_nut_en, // Term name
				'wheel-lug-nut', // Taxonomy name
				[
			...
			if (!( is_wp_error( $new_term ) )) {
				$term_exists = term_exists($wheel_lug_nut_ar, 'wheel-lug-nut');

						if ($term_exists) {
							$wheel_lug_nut_ar = $wheel_lug_nut_ar."(ar)";
						}
		...
		$success_msg = 'Wheel Lug Nut Added Successfully';
	...
	<div class="main-panel">
		...
		<h4 class="card-title"><?php echo pll__('Add Wheel Lug Nut');?></h4>
		...
		<input type="text" class="form-control  "  name="wheel_lug_nut" id='wheel_lug_nut' value="" >
		</div>
		</div>
			<div class="col-md-6">
		<div class="form-group">
		<label class="bmd-label-floating"><?php echo pll__('Wheel Lug Nut');?>(AR)</label>
		<input type="text" class="form-control  "  name="wheel_lug_nut_ar" id='wheel_lug_nut_ar' value="" >
		</div>
		</div>
		...
		<button type="submit" class="btn btn-primary pull-right" id="save_wheel_lug_nut" name="save_wheel_lug_nut" onclick ='return validate_form_brand()' ><?php echo pll__('Save Wheel Lug Nut');?></button>
...
<script type="text/javascript">
 function  validate_form_brand(){
		 ...
		 $('.error').remove()
 wheel_lug_nut = jQuery('#wheel_lug_nut').val()
  wheel_lug_nut_ar = jQuery('#wheel_lug_nut_ar').val()
				 if(wheel_lug_nut==""){
				jQuery('#wheel_lug_nut').parent().append("<span class='error'><?php echo pll__('Please Enter Wheel Lug Nut Name In English !');?></span>")
				jQuery('#wheel_lug_nut').focus();
				error = 1;
			}
				 if(wheel_lug_nut_ar==""){
				jQuery('#wheel_lug_nut_ar').parent().append("<span class='error'><?php echo pll__('Please Enter Wheel Lug Nut Name In Arabic !');?></span>")
				jQuery('#wheel_lug_nut_ar').focus();
				error = 1;
			}
			if(error ==0){
return true			}else{
