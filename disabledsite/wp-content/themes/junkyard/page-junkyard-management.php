<?php 
/*
*Template Name: Junkyard Management
*/ 
get_header('user');
			$user_role = array();
...
while ( have_posts() ) : the_post(); 
	...
	if(isset($_REQUEST['id'])){
		if(get_current_user_id() !=0){
			...
			$id = intval($id);
			$junkyard_id_en = pll_get_post($id,'en' );
			$junkyard_id_ar = pll_get_post($id,'ar' );
			if($action=="delete"){
				...
				update_post_meta(  $junkyard_id_en, 'deleted_'.date('d-m-Y_H_i_s'), get_current_user_id() );
				 
		$success_msg = pll__('Junkyard Deleted Successfully!');
			}
		}
	}
	?> 
		...
		<nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top ">
			<div class="container-fluid">
				<div class="navbar-wrapper">
					<a class="navbar-brand" href="javascript:;"><?php echo pll__('Junkyard');?></a>
				</div>
		...
		<div class="content">
			<div class="container-fluid">
				<div class="row">
					<div class="col-md-12">
						<?php 	
						$page = get_page_by_title('Add Junkyard');
						$post_id = pll_get_post( $page->ID,pll_current_language() );
						$link = get_permalink($post_id);?>
						<button type="button" class="btn btn-primary pull-right add-item" onclick="location.href='<?php echo $link;?>'">+ <?php echo pll__('Add Junkyard');?><div class="ripple-container"></div></button> 
						<?php if($success_msg !='')
						{
						?>
						...
						<div class="card">
							<div class="card-header card-header-primary">
								<h4 class="card-title"><?php echo pll__('Stores');?></h4>
							...
							<div class="card-body">
								...
								<thead class=" text-primary">
									<tr>
										...
										<th style='width:250px;'>
										<?php echo pll__('Phone');?>
										</th> 
										<th style='width:250px;'>
										<?php echo pll__('Address');?>
										</th> 
										<th style='width:250px;'>
										<?php echo pll__('Username');?>
										</th> 
										<th style='width:250px;'>
										<?php echo pll__('Password');?>
										</th> 
								...
								<?php 	$page = get_page_by_title('Edit Junkyard');
								$post_id = pll_get_post( $page->ID,pll_current_language() );
								$link = get_permalink($post_id);
								$dpage = get_page_by_title('Junkyard Management');
								$dpost_id = pll_get_post( $dpage->ID,pll_current_language() );
								$dlink = get_permalink($dpost_id);
								$args = array(	'post_type' => array('junkyard'),
								'posts_per_page' => -1,  
								'post_status'	=>'publish',
								'orderby'   => 'ID',
								'order' => 'DESC',
								...
								if($loop->have_posts()) {
									...
									$password = get_user_meta($user_id,'uqbttdrow',true);
                        
											?>
										<tr>
											<td><?php echo get_the_title();?></td>
											<td><?php echo get_field('phone');?></td>
											<!--<td><?php if(get_field('delivery_available',get_the_ID())=="Yes"){ ?>
											Yes
											<?php }else{ ?>
											No
											<?php } ?>
											...
											<td username=<?php echo $username;?> class="copyusername" style="cursor:pointer"><?php echo $username;?></td>
											<td password="<?php echo $password;?>" class="copypass"  style="cursor:pointer">*******</td>
											 
											<td><a href='<?php echo $link;?>?id=<?php echo get_the_ID();?>'><i class="material-icons">edit</i></a>&nbsp;<a href='<?php echo $dlink;?>?action=delete&id=<?php echo get_the_ID();?>' onclick="return confirm('<?php echo pll__('Are you sure you want to delete the junkyard?');?>')"><i class="material-icons">cancel</i></a>
											</td>
										</tr>
										<?php 
									endwhile;
...
<script type="text/javascript">//<![CDATA[
 ...
 "order": [[0, 'desc']],   
		...
		extend: 'excelHtml5',
		autoFilter: true,
		sheetName: '<?php echo pll__('Junkyards');?>',
		exportOptions: {
		 
										 columns: [0 ] // Specify column indices to export (e.g., first three columns)
 ...
 
  {
                ...
                columns: [0,1,2,3] // Specify column indices to export (e.g., first three columns)
							...
							title: '<?php echo pll__('Junkyards');?>',
							text: 'PDF', 
							titleAttr: 'PDF',
						   pageSize: 'A4',   
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // Select all td elements with the class 'copyusername'
        const tdElements = document.querySelectorAll('.copyusername');
        ...
        const tdpElements = document.querySelectorAll('.copypass');

        // Loop through each td element
        tdpElements.forEach(td => {
            td.addEventListener('click', function () {
                // Get the text content of the clicked td
                const textToCopy =  td.getAttribute('password');
