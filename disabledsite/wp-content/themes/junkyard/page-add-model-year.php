...
/*
*Template Name: Add Model Year Template
*/ 
get_header('user'); 
			$user_role = array();
...
while ( have_posts() ) : the_post(); 
	...
	if(isset($_REQUEST['save_model_year'])){ 
		...
		$model_year_en = sanitize_text_field($_REQUEST['model_year']);
		$model_year_ar = sanitize_text_field($_REQUEST['model_year_ar']);
 
			// Add the term to the taxonomy
			$new_term = wp_insert_term(
				$model_year_en, // Term name
				'model-year', // Taxonomy name
				[
		...
		$success_msg =   pll__('Model Year Added Successfully');
			}else{
				$err_msg =  pll__('Model Year Exists');
			}
	} 
	?> 
	<style>
	...
	<div class="main-panel">
		...
		<h4 class="card-title"><?php echo pll__('Add Model Year');?></h4>
		...
		<input type="text" class="form-control  "  name="model_year" id='model_year' value="" >
		</div>
		</div>
			<div class="col-md-6">
		<div class="form-group">
		<label class="bmd-label-floating"><?php echo pll__('Model Year');?>(AR)</label>
		<input type="text" class="form-control  "  name="model_year_ar" id='model_year_ar' value="" >
		</div>
		</div>
		...
		<button type="submit" class="btn btn-primary pull-right" id="save_model_year" name="save_model_year" onclick ='return validate_form_brand()' ><?php echo pll__('Save Model Year');?></button>
...
<script type="text/javascript">
 function  validate_form_brand(){
		 ...
		 $('.error').remove()
 model_year = jQuery('#model_year').val()
  model_year_ar = jQuery('#model_year_ar').val()
				 if(model_year==""){
				jQuery('#model_year').parent().append("<span class='error'><?php echo pll__('Please Enter Model Year Name In English !');?></span>")
				jQuery('#model_year').focus();
				error = 1;
			}
				 if(model_year_ar==""){
				jQuery('#model_year_ar').parent().append("<span class='error'><?php echo pll__('Please Enter Model Year Name In Arabic !');?></span>")
				jQuery('#model_year_ar').focus();
				error = 1;
			}
			if(error ==0){
return true			}else{
