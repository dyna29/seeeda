<?php 
/*
*Template Name: Edit Wheel Size Template
*/ 
get_header('user');
			$user_role = array();
...
while ( have_posts() ) : the_post(); 
 ...
 $ar_term = pll_get_term($_REQUEST["id"], 'ar');	
	...
	if(isset($_REQUEST['save_wheel_size'])){  
			$wheel_size_name_en = sanitize_text_field($_REQUEST['wheel_size']);
		$wheel_size_name_ar = sanitize_text_field($_REQUEST['wheel_size_ar']);
 
	$args = array(
    'name' => $wheel_size_name_en,
	'parent'=>0,
);
$updated_term = wp_update_term( $en_term, 'wheel-size', $args );	
			 
	$args = array(
    'name' => $wheel_size_name_ar,
	'parent'=>0,
);
$updated_term = wp_update_term( $ar_term, 'wheel-size', $args );	
  
		 
			 
		$success_msg = 'Wheel Size Updated Successfully';
	} 
if ($en_term) {
    $term = get_term($en_term, 'wheel-size'); // Replace 'wheel_size' with your actual taxonomy name if different
    if ($term && !is_wp_error($term)) {
       $wheel_sizeen_term  = $term->name; // Outputs the translated term name
    }   

	} 
	
	if ($ar_term) {
    $term = get_term($ar_term, 'wheel-size'); // Replace 'wheel_size' with your actual taxonomy name if different
    if ($term && !is_wp_error($term)) {
       $wheel_sizear_term  = $term->name; // Outputs the translated term name
    }   
				...
				<div class="row">
					...
					<div class="card">
						<div class="card-header card-header-primary">
							<h4 class="card-title"><?php echo pll__('Edit Wheel Size');?></h4>
							<p class="card-category"></p>
						</div>
						<?php 
						$_associated_junkyard = get_post_meta( $wheel_size_id_en,'_associated_junkyard',true);  
						$featuredimage = wp_get_attachment_image_src( get_post_thumbnail_id( $wheel_size_id_en ), 'full' ); 	
						?>
						<div class="card-body">
							<form action='' method='post' class='frm-addeditwheel_size'    enctype='multipart/form-data'>
						 
								<div class="row">
									<div class="col-md-6">
										<div class="form-group">
											<label class="bmd-label-floating"><?php echo pll__('Wheel Size');?>(EN)</label>
											<input type="text" class="form-control  "  name="wheel_size" id='wheel_size' value="<?php echo $wheel_sizeen_term;?>" >
										</div>
									</div> 
									<div class="col-md-6">
										<div class="form-group">
											<label class="bmd-label-floating"><?php echo pll__('Wheel Size');?>(AR)</label>
											<input type="text" class="form-control  "  name="wheel_size_ar" id='wheel_size_ar' value="<?php echo $wheel_sizear_term;?>" >
										</div>
									</div> 
								...
								<button type="submit" class="btn btn-primary pull-right" id="save-wheel_size" name="save_wheel_size" onclick ='return validate_form_wheel_size()'  ><?php echo pll__('Save Wheel Size');?></button>
...
function  validate_form_wheel_size(){
		 error = 0

		 $('.error').remove()
 wheel_size_name = jQuery('#wheel_size_name').val()
  wheel_size_name_ar = jQuery('#wheel_size_name_ar').val()
				 if(wheel_size_name==""){
				jQuery('#wheel_size_name').parent().append("<span class='error'><?php echo pll__('Please Enter Wheel Size Name In English !');?></span>")
				jQuery('#wheel_size_name').focus();
				error = 1;
			}
				 if(wheel_size_name_ar==""){
				jQuery('#wheel_size_name_ar').parent().append("<span class='error'><?php echo pll__('Please Enter Wheel Size Name In Arabic !');?></span>")
				jQuery('#wheel_size_name_ar').focus();
				error = 1;
			}
