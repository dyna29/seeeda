...
/*
*Template Name: Add Brand Template
*/ 
get_header('user'); 
			$user_role = array();
...
while ( have_posts() ) : the_post(); 
        ...
        $manufacturing_origin = sanitize_text_field($_REQUEST['manufacturing_origin']);
			...
			$new_term = wp_insert_term(
				...
				[
					...
					'parent' => 0, // Set parent term ID if needed; 0 means top-level
					'slug' => sanitize_title( $brand_name_ar )."-ar",
				]
			);
			...
			if($ret = wp_upload_bits($filename, null, file_get_contents($_FILES["featured_image"]["tmp_name"])))
			{
				...
				if (!$ret['error']) {
					...
					if (!is_wp_error($attachment_id)){
					    ...
					    update_field('thumbnail', $attachment_id, "term_".$brand_ar);
						 update_field('thumbnail', $attachment_id, "term_".$brand_en );
					}
				}
			}  
		}
		 
		$success_msg = pll__('Brand Added Successfully');
		
			}else{
				
		$err_msg = 'Brand Exists';
			}
	} 
	?> 
	<style>
	...
	<div class="main-panel">
		...
		<h4 class="card-title"><?php echo pll__('Add Brand');?></h4>
