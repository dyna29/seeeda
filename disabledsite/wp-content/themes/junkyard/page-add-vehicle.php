...
/*
*Template Name: Add Vehicle Template
*/ 
get_header('user'); 
while ( have_posts() ) : the_post(); 
	...
	if(isset($_REQUEST['save_vehicle'])){ 
		...
		$vehicle_id_en =  wp_insert_post( $post_update );
		pll_set_post_language($vehicle_id_en, 'en');
		$post_update = array( 
			'post_title' => $vehicle_name_ar,
			'post_type' =>'vehicle','post_status'=>'publish','lang'=>'ar',
		);
		$vehicle_id_ar =  wp_insert_post( $post_update );
		pll_set_post_language($vehicle_id_ar, 'ar');
		$arr = array('en'=>$vehicle_id_en,'ar'=>$vehicle_id_ar );
	...
	<div class="main-panel">
		...
		<h4 class="card-title"><?php echo pll__('Add Vehicle');?></h4>
...
?>
	...
	<!--
		...
		<input type="text" class="form-control  "  name="vehicle_name_ar" id='vehicle_name_ar' value="" >
		</div>
		</div>
