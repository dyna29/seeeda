<?php 
/*
*Template Name: Edit Spare Part Type Template
*/ 
get_header('user');
			$user_role = array();
...
while ( have_posts() ) : the_post(); 
 ...
 $args = array(
    'name'   => $spare_part_type_name_en,  // Set the new name for the term
    'parent' => 0,     // Set parent to 0 to make it a top-level term
);
 
 
$en = 	wp_update_term($en_term, 'spare-part-type', [
	'name' => $spare_part_type_name_en,
]);	
 
$ar = 	wp_update_term($ar_term, 'spare-part-type', [
	'name' => $spare_part_type_name_ar,
]);	
	 $file = $_FILES['featured_image'];	 
		...
		$success_msg = 'Spare Part Type Updated Successfully';
	} 
	if ($en_term) {
		$term = get_term($en_term, 'spare-part-type'); // Replace 'spare_part_type' with your actual taxonomy name if different
	...
	if ($ar_term) {
		$term = get_term($ar_term, 'spare-part-type'); // Replace 'spare_part_type' with your actual taxonomy name if different
		if ($term && !is_wp_error($term)) {
		   $spare_part_typear_term  = $term->name; // Outputs the translated term name
		}   
	
		} 
	$cur_term = pll_get_term($_REQUEST["id"], pll_current_language()); 
	 
	 $cur_term = get_term($cur_term, 'spare-part-type');
				...
				<div class="row">
					...
					<div class="card">
						<div class="card-header card-header-primary">
							<h4 class="card-title"><?php echo pll__('Edit Spare Part Type');?></h4>
