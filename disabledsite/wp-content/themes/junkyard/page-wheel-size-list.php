...
while ( have_posts() ) : the_post(); 
	...
	if(isset($_REQUEST['id'])){
		if(get_current_user_id() !=0){
			...
			if($action=="delete"){
			wp_delete_term( $spare_part_id_en,  'wheel-size' );
			wp_delete_term( $spare_part_id_ar,  'wheel-size' ); 
				 
		$success_msg = pll__('Wheel Size Deleted Successfully!');
			}
		}
	}
	?> 
		...
		<nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top ">
			<div class="container-fluid">
				<div class="navbar-wrapper">
					<a class="navbar-brand" href="javascript:;"><?php echo pll__('Wheel Size');?></a>
				</div>
		...
		<div class="content">
			<div class="container-fluid">
				<div class="row">
					<div class="col-md-12">
						<?php 	
						$page = get_page_by_title('Add Wheel Size');
						$post_id = pll_get_post( $page->ID,pll_current_language() );
						$link = get_permalink($post_id);?>
						<button type="button" class="btn btn-primary pull-right add-item" onclick="location.href='<?php echo $link;?>'">+ <?php echo pll__('Add Wheel Size');?><div class="ripple-container"></div></button> 
						<?php if($success_msg !='')
						{
						?>
						...
						<div class="card">
							<div class="card-header card-header-primary">
								<h4 class="card-title"><?php echo pll__('Wheel Size List');?></h4>
							...
							<div class="card-body">
								...
								$page = get_page_by_title('Edit Wheel Size');
								$post_id = pll_get_post( $page->ID,pll_current_language() );
								$link = get_permalink($post_id);
...
?>
										<tr> 
											...
											<td><a href='<?php echo $link;?>?id=<?php echo $term->term_id;?>'><i class="material-icons">edit</i></a>&nbsp;<a href='<?php echo $dlink;?>?action=delete&id=<?php echo $term->term_id;?>' onclick="return confirm('Are you sure you want to delete the wheel size?')"><i class="material-icons">cancel</i></a>
											</td>
										</tr>
										<?php 
									    }
...
<script type="text/javascript">//<![CDATA[
 ...
 "order": [[0, 'desc']],   
		...
		extend: 'excelHtml5',
		autoFilter: true,
		sheetName: '<?php echo pll__('Wheel Size List');?>',
		exportOptions: {
 ...
 
  {
                ...
                columns: [0] // Specify column indices to export (e.g., first three columns)
							...
							title: '<?php echo pll__('Wheel Size List');?>',
							text: 'PDF', 
							titleAttr: 'PDF',
						   pageSize: 'A4',   
