...
/*
*Template Name: Add Spare Part Sub Type Template
*/ 
get_header('user'); 
			$user_role = array();
...
while ( have_posts() ) : the_post(); 
	...
	if(isset($_REQUEST['save_spare_part_type'])){ 
		...
		$spare_part_sub_type_name_ar = sanitize_text_field($_REQUEST['spare_part_type_name_ar']);
			...
			$new_term = wp_insert_term(
				$spare_part_sub_type_name_en, // Term name
				'spare-part-type', // Taxonomy name
				[
				   
					'parent' => $spare_part_type, // Set parent term ID if needed; 0 means top-level
				]
			);
			
			 pll_set_term_language($new_term['term_id'],'en'); 
			 
			  $ar_spare_part_type = pll_get_term($spare_part_type, 'ar');	
			 $spare_part_type_en = $new_term['term_id'];
		  
		 
			// Add the term to the taxonomy
			$new_term = wp_insert_term(
				$spare_part_sub_type_name_ar, // Term name
				'spare-part-type', // Taxonomy name
				[
				   
					'parent' => $ar_spare_part_type, // Set parent term ID if needed; 0 means top-level
				]
			);
		...
		$success_msg = 'Spare Part Sub Type Added Successfully';
	} 
	?> 
	<style>
	...
	<div class="main-panel">
		...
		<h4 class="card-title"><?php echo pll__('Add Spare Part Sub Type');?></h4>
...
if (!empty($spare_part_type_terms) && !is_wp_error($spare_part_type_terms)) :
    foreach ($spare_part_type_terms as $parent_term) {
        ...
        $spare_part_type_id = esc_html($parent_term->term_id) ;
				...
				?>
		<option value="<?php echo $spare_part_type_id;?>"><?php echo $spare_part_type;?></option> 
		<?php
...
?>
		...
		<div class="col-md-6">
		<div class="form-group">
		<label class="bmd-label-floating"><?php echo pll__('Spare Part Sub Type');?>(EN)</label>
		<input type="text" class="form-control  "  name="spare_part_type_name" id='spare_part_type_name' value="" >
		</div>
		</div>
			<div class="col-md-6">
		<div class="form-group">
		<label class="bmd-label-floating"><?php echo pll__('Spare Part Sub Type');?>(AR)</label>
