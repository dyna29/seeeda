		...
		<div class="content">
			<div class="container-fluid">
				<div class="row">
					<div class="col-md-12">
						<?php 	
						$page = get_page_by_title('Add Model');
						$post_id = pll_get_post( $page->ID,pll_current_language() );
						$link = get_permalink($post_id);?>
						<button type="button" class="btn btn-primary pull-right add-item" onclick="location.href='<?php echo $link;?>'">+ <?php echo pll__('Add Model');?><div class="ripple-container"></div></button> 
						<?php if($success_msg !='')
						{
						?>
						...
						<div class="card">
							<div class="card-header card-header-primary">
								<h4 class="card-title"><?php echo pll__('Models');?></h4>
							...
							<div class="card-body">
								...
								$page = get_page_by_title('Edit Model');
								$post_id = pll_get_post( $page->ID,pll_current_language() );
								$link = get_permalink($post_id);
								// Get all terms where parent is 0
								$terms = get_terms(array(
									'taxonomy'   => 'brand',     // Replace 'brand' with your custom taxonomy name
								...
								if (!empty($terms) && !is_wp_error($terms)) {
									foreach ($terms as $term) { 
												 if ($term->parent != 0) { 
									$thumbnail = get_field('thumbnail', get_term($term->parent, 'brand'));
?>
										<tr>
											<td><img src="<?php echo $thumbnail;?>" class="list-thumb"></td>
											<td><?php echo get_term($term->parent, 'brand')->name;?></td>
											<td><?php echo $term->name;?></td>
									 
											<td><a href='<?php echo $link;?>?id=<?php echo $term->term_id;?>'><i class="material-icons">edit</i></a>&nbsp;<a href='<?php echo $dlink;?>?action=delete&id=<?php echo $term->term_id;?>' onclick="return confirm('Are you sure you want to delete the model?')"><i class="material-icons">cancel</i></a>
											</td>
										</tr>
										<?php 
									    }
...
<script type="text/javascript">//<![CDATA[
 ...
 "order": [[0, 'desc']],   
		...
		extend: 'excelHtml5',
		autoFilter: true,
		sheetName: '<?php echo pll__('Brands');?>',
		exportOptions: {
 ...
 
  {
							extend: 'pdf',
						 exportOptions: {
                columns: [1,2] // Specify column indices to export (e.g., first three columns)
            },
							title: '<?php echo pll__('Brands');?>',
							text: 'PDF', 
							titleAttr: 'PDF',
						   pageSize: 'A4',   
