<?php 
/*
*Template Name: Vehicle Management
*/ 
get_header('user');
while ( have_posts() ) : the_post(); 
	...
	if(isset($_REQUEST['id'])){
		if(get_current_user_id() !=0){
			...
			$id = intval($id);
			$vehicle_id_en = pll_get_post($id,'en' );
			$vehicle_id_ar = pll_get_post($id,'ar' );
			if($action=="delete"){
				...
				update_post_meta(  $vehicle_id_en, 'deleted_'.date('d-m-Y_H_i_s'), get_current_user_id() );
				 
		$success_msg = pll__('Vehicle Deleted Successfully!');
			}
		}
	}
	?> 
		...
		<nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top ">
			<div class="container-fluid">
				<div class="navbar-wrapper">
					<a class="navbar-brand" href="javascript:;"><?php echo pll__('Vehicle');?></a>
				</div>
		...
		<div class="content">
			<div class="container-fluid">
				<div class="row">
					<div class="col-md-12">
						<?php 	
						$page = get_page_by_title('Add Vehicle');
						$post_id = pll_get_post( $page->ID,pll_current_language() );
						$link = get_permalink($post_id);?>
						<button type="button" class="btn btn-primary pull-right add-item" onclick="location.href='<?php echo $link;?>'">+ <?php echo pll__('Add Vehicle');?><div class="ripple-container"></div></button> 
						<?php if($success_msg !='')
						{
						?>
						...
						<div class="card">
							<div class="card-header card-header-primary">
								<h4 class="card-title"><?php echo pll__('Vehicles');?></h4>
							...
							<div class="card-body">
								...
								<?php 	$page = get_page_by_title('Edit Vehicle');
								$post_id = pll_get_post( $page->ID,pll_current_language() );
								$link = get_permalink($post_id);
								$dpage = get_page_by_title('Vehicle Management');
								...
								$dlink = get_permalink($dpost_id);
								 if( (in_array("junkyards_administrator", $user_role)) )
										{
								$args = array(	'post_type' => array('vehicle'),
								'posts_per_page' => -1,  
								'post_status'	=>'publish',
								...
								if($loop->have_posts()) {
									...
									while($loop->have_posts()) : $loop->the_post();
										$i++; 
										  $terms = get_the_terms(get_the_ID(), 'brand');
											...
											if (!empty($model_year_terms) && !is_wp_error($model_year_terms)) {
												// Assuming only one model year term per vehicle
												$model_year = $model_year_terms[0]->name;
											}
											$selected_junkyard = get_post_meta($post->ID, '_associated_junkyard', true);
												$selected_junkyard = pll_get_post($selected_junkyard ,pll_current_language() );
											?>
										<tr>
										...
										{?>
											...
											<?php } ?>
											<td><a href='<?php echo $link;?>?id=<?php echo get_the_ID();?>'><i class="material-icons">edit</i></a>&nbsp;<a href='<?php echo $dlink;?>?action=delete&id=<?php echo get_the_ID();?>' onclick="return confirm('Are you sure you want to delete the vehicle?')"><i class="material-icons">cancel</i></a>
											</td>
										</tr>
										<?php 
									endwhile;
...
<script type="text/javascript">//<![CDATA[
 ...
 "order": [[0, 'desc']],   
		...
		extend: 'excelHtml5',
		autoFilter: true,
		sheetName: '<?php echo pll__('Vehicles');?>',
		exportOptions: {
